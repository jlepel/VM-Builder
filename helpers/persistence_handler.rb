...
class Datafile
  ...
  property :target, String, :length => 255 # A text block, for longer string data.
...
###################################################################################################################
      ...
      begin
        ...
        if !files.nil?
          files.each do |file_data|
            Datafile.create(:name => file_data[:file][:filename], :source => get_vm_installpath + name, :target => file_data[:path])
            Machinefile.create(:machine_id => get_machine_id(name), :datafile_id => get_file_id(file_data[:file][:filename]))
          end
        end
      rescue
        t.rollback
  ...
  def save_import(machine_name, files)
    Machine.transaction do |t|
      begin
        Machine.first_or_create(:name => machine_name, :ip => '', :description => 'IMPORT', :vmimage_vm_name => 'precise32', :status => 0)
        if !files.nil?
          files.each do |file_data|
            Datafile.create(:name => file_data[:file][:filename], :source => get_vm_installpath + machine_name, :target => ' ')
            Machinefile.create(:machine_id => get_machine_id(machine_name), :datafile_id => get_file_id(file_data[:file][:filename]))
          end
        end
      rescue
        t.rollback
...
#                                          Software                                                               #
###################################################################################################################
  def add_software(program, command, desc)
...
###################################################################################################################
  ...
  def save_package(name, desc, selection, files)
    Software.transaction do |t|
      begin
        Software.create(:name => name, :command => '', :description => desc, :package => 1)
        selection.each do |software|
          Package.create(:source_id => get_software_id(name), :sub_id => get_software_id(software))
        end

        if !files.nil?
          files.each do |file_data|
            Datafile.create(:name => file_data[:file][:filename], :source => get_file_path, :target => file_data[:path])
            Softwarefile.create(:software_id => get_software_id(name), :datafile_id => get_file_id(file_data[:file][:filename]))
          end
        end
      rescue
        t.rollback
  ...
  def update_package(id, package_name, description, selection, files)
    Software.transaction do |t|
      begin
        ...
        selection.each do |software|
          Package.create(:source_id => id, :sub_id => get_software_id(software)).update(:sub_id => get_software_id(software))
        end
        if !files.nil?
          files.each do |file_data|
            Datafile.create(:name => file_data[:file][:filename], :source => get_vm_installpath + name, :target => file_data[:path])
  ...
  def get_all_packagefiles(id)

    file_array = Array.new
    file_association = Softwarefile.all(:software_id => id)
    file_association.each do |file|
      file_array << get_file_by_id(file.datafile_id)
    end
    file_array
  end
